...
# Define URL for the farmer field data
...
for farmer in response_api.json():
    ...
    # For each crop type, determine the crop season based on planting and harvesting date and determine the kc values.
    if (
        crops == "Habanero Peppers"
    ):  # this part is crop specific, this is still needed in the magda file
        # but  here also the ndvi algorithm needs to be added
        kc1 = 0.6
        kc2 = 1.05
        kc3 = 1.05
    ...
    # Get different time variables in EE format. But don't understand why create a today (based on current day),
    # as this is all historic data.
    ...
    # Loads following datasets
    # - WAPOR RET: Reference Evapotranspiration
    # - GPM: Global Precipitation Data
    # - CFSv2: Precipitation and Temperature Forecast Data (Is it seasonal?)
    ...
    # Point is defined as long/lat coordinates from farmer data.
    # Create a buffer around the point with a radius of 30m and then clips the image to this buffer.
    # Select only the WAPOR data for the specific dates and clip it to the buffer.
    # here 30 means average of the point + surrounding pixels
    ...
    # Take all the wapor_filtered images and divide them by 10 and create a new band called corrected and add this new band to the image.
    def func_cri(img):
    ...
    # But it gets the corrected image collection, filters it by days of the year
    # and then calculates the mean of the buffered point for each day in the season with 2 decimals
    # behind the comma.
    ...
    """
    Understand very little here.
    What it seems to do is that for every crop stage it calculates first number of days in that stage (based on start and end dates from
    JSON, converted to milliesecond.). Then gives every day in that season the number it has in the year (1 to 365). So you get an EE
    list with all the DOYs in the season. Then it calculates the average RET for each day in that season, and then multiplies it with
    the respective kc value.
    Then there's a function that sets a new property 'Date' to the image, which is the date of the image. Don't understand this yet.
    """
    # --------------------------------------------------------------------------------------------------------------------
    ...
    # joined is csv of 125 days of the season
    Historical_crop_schedule = innerJoin.map(func_nlb)
    ...
    def poi_mean(img):
        ...
        Etref = img.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=clip_geo, scale=30
        ).get("1 ETref in mm/day")
        Etc = img.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=clip_geo, scale=30
        ).get("2 ETc in mm/day")
        irrvol = img.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=clip_geo, scale=30
        ).get("3 Irrigation needs in m3/day")
        irrtime = img.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=clip_geo, scale=30
    ...
    url_tahmo = (
        "https://smartirrigation.tahmo.org/sosia/nearestwetnessreport/" + pointstring
    )
    ...
    if result:
        ...
        days_hind = ee.List.sequence(weekagomillis, todaymillis, millisInDay).map(
            func_iyz
        )
    ...
    # ---------------------------------- This is the important part, when no stations are available -------------------------------------------------------------------------
        ...
        # Hindcast CFSv2. Used to check if mean RET was correct. If not
        # -9 and -2 are chosen, because the hindcast only gives information of 2 days ago, so we are using a moving window loop here
        weekago = today.advance(-9, "day")
        ...
        def daily_mean(dayOffset):
            ...
            # hier ergens bij deze dingen gaat het mis; string is 085, je wil 85
            return (
                CFSR_tot.filterDate(start, end)
                .mean()
                .set("Date", dateIm)
                .set("DOY", doyIm)
            )
        ...
        daily_mean = ee.ImageCollection(
            ee.List.sequence(0, numberOfDays.subtract(1)).map(daily_mean)
        )
        ...
        def CFSR_Temp_Max(dayOffset):
            ...
            # .set('DOY', start.format('DDD'))
            return (
                CFSR_Temp.filterDate(start, end)
                .max()
                .set("Date", start.format("YYYY-MM-dd"))
            )

        CFSR_Temp_Max = ee.ImageCollection(
            ee.List.sequence(0, numberOfDays.subtract(1)).map(CFSR_Temp_Max)
        )

        Temp_max = CFSR_Temp_Max.map(
        ...
        CFSR_Temp_Min = ee.ImageCollection(
            ee.List.sequence(0, numberOfDays.subtract(1)).map(CFSR_Temp_Min)
        )

        Temp_min = CFSR_Temp_Min.map(
            lambda image: image.select("Temperature_height_above_ground").rename(
                "Min Temperature"
            )
        )
        ...
        innerJoin_min = ee.ImageCollection(
            simpleJoin.apply(Temp_max, Temp_min, joinFilter)
        )

        tempmaxmin = innerJoin_min.map(
            lambda feature: ee.Image.cat(
                feature.get("primary"), feature.get("secondary")
            )
        )

        innerJoin_max = ee.ImageCollection(
            simpleJoin.apply(daily_mean, tempmaxmin, joinFilter)
        )

        CFSR_total = innerJoin_max.map(
            lambda feature: ee.Image.cat(
                feature.get("primary"), feature.get("secondary")
            )
        )
    ...
    # --------- Go from ETref to ETc -----> multiplying by kc  -----------------------------
    ...
    # WAPOR only used here as data structure ------------------------
    def kcdev(image):
        return (
            image.multiply(0)
            .add(kc1)
