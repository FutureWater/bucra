...
unique_combis = [f"Land_Suitability_{x}_{y}" for x in [
    "Average"] + T_perc_names for y in ["Average"] + P_perc_names]
...
communes_df = communes_shp[['NAME_1', 'NAME_2', 'NAME_3']].rename(
    columns={'NAME_1': 'Province', 'NAME_2': 'Municipality', 'NAME_3': 'Commune'})
...
for i, row in final_forecast_df.iterrows():
    final_forecast_df.loc[i, 'Province'] = communes_df['Province'][communes_df['Commune']
                                                                   == row['Commune']].values[0]
...
for script in list_scripts[n:]:
    for name in provinces_names:
        ...
        print(
            f"Province: {province_name} & script: {os.path.basename(script)}")
        # Execute the script (you'll need to implement a way to translate R script into Python functions)

        # Clean up temporary files after processing
        temp_files = glob.glob(os.path.join(temp_dir, "*.grd")) + \
            glob.glob(os.path.join(temp_dir, "*.gri"))
