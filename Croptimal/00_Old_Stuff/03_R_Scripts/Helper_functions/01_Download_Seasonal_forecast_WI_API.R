...
#import_info = read.csv(paste0("./Projects/MavoDiami/Seasonalforecast/Seasonal_structure.csv"))
import_info = read.csv(paste0("C:\\Users\\Lisa\\Documents\\Projects\\2019019_MavoDiami\\Seasonalforecast\\Seasonal_structure.csv"))
...
getMet <- function(url, endpoint, authkey, import_info, attempts){
  ...
  while(is.null(request) && attempt <= attempts) {
    ...
    try(
      request <- withTimeout(GET(url = url_st, add_headers(.headers = c('authkey'= authkey))),timeout = 10, onTimeout = "silent")
    )
  } 
  response <- RJSONIO::fromJSON(content(request, type="text")) # Note 20220314: line taken from Rick seems to fix it
}
