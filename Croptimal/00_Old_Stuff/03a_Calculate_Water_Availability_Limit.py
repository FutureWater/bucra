...
VARIABLES = ['P05', 'P25', 'Mean']  # Percentiles to process
...
for p_idx, var in enumerate(VARIABLES):
    print(f"Processing water availability with {var} Precipitation...")

    # Create subdirectories
    os.makedirs(os.path.join(NEW_RESULTS_SUBDIR,
                "Rainfall", var), exist_ok=True)
    os.makedirs(os.path.join(NEW_RESULTS_SUBDIR, "Water", var), exist_ok=True)
    ...
    for _, crop_data in CROPPING_CAL.iterrows():
        ...
        p_files = [f for f in INPUT_FILES_P if var in f and any(
            m in os.path.basename(f) for m in month_abbrs)]
        ...
        out_paths = {
            ...
            "P": os.path.join(NEW_RESULTS_SUBDIR, "Rainfall", var, f"{var}_{crop_name}_{season_label}.tif"),
            "Water": os.path.join(NEW_RESULTS_SUBDIR, "Water", var,
                                  f"Water_availability_{var}_{crop_name}_{season_label}_higher_than_{limit}.tif")
        }
        ...
        print(f"    Processed {crop_name} in {season_label} for {var}")
