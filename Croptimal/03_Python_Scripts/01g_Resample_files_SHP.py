...
HHS_FOLDER = r"C:\Users\thomv\FutureWater Dropbox\Team\Data\Global\Soil\HiHydroSoil_250m\Top_Subsoil"
...
INPUT_FILES = [file for var in VAR_NAMES for file in glob.glob(os.path.join(
    r'C:\Users\thomv\FutureWater Dropbox\Team\Data\Global\Soil\HiHydroSoil_250m\Top_Subsoil', var, '*.tif'))]
if not INPUT_FILES:
...
province_shp_buffer = province_shp_reproj.buffer(20000)
...
for i, var in enumerate(VAR_NAMES):
    input_files = [file for file in INPUT_FILES if var in file]
    ...
    for file in input_files:
        with rasterio.open(file) as src:
            ...
            province_shp_window_crs = province_shp_buffer.to_crs(src.crs)
            ...
            reproject(
                ...
                dst_crs=DEM_CRS,
                resampling=Resampling.bilinear  # You can choose another resampling method
            )
            ...
            # ALTERNATIVE METHOD: Ensure all cells that are no data in DEM are also no data for HHS
            masked_data = np.where(DEM_DATA != NO_DATA_VALUE, destination_array, NO_DATA_VALUE) # convert resampled data by 0.0001 to get correct HHS units
            converted_data = masked_data.copy()
            if var == 'Ksat':
                converted_data[converted_data != NO_DATA_VALUE] *= 0.001 # Divide by 10.000 (required for SSH data) and then multiply with 10 to get proper units (go from cm/day to mm/day)
            else:
                converted_data[converted_data != NO_DATA_VALUE] *= 0.0001 # Divide by 10000 to get proper units (required for HHS data)

            # Write the final masked result to file
            top_sub = os.path.basename(file).split("_")[-1].split(".")[0]
            ...
            with rasterio.open(OUTPUT_PATH, 'w', **masked_profile) as dst:
                dst.write(converted_data.astype(rasterio.float32))
